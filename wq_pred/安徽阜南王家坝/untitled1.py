...
save_xls_file(test_dataout1,"t2a")
...
h1_size=10#int(data0.shape[1]/3)
...
with tf.Session() as sess:
    ...
    starttime = time.time()
    output0=[]
    print(test_datain1.shape[0])
    for i in range(6,test_datain1.shape[0]-1):

        #online training
        for t in range(10):     
            for j in range(5):
                t_temx=test_datain1[i-1-j,:]
                t_temy=test_datain1[i-1-j+1,:]
                bx=np.array([t_temx])
                by=np.array([t_temy])
                sess.run(train,feed_dict={x_:bx,y_:by})
